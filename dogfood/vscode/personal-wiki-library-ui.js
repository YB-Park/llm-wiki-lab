...
async function registerExternalStore(context, folder) {
  ...
  const confirmed = context.extensionMode === vscode.ExtensionMode.Test
    ...
    : await vscode.window.showWarningMessage(
      `Register “${displayName.trim()}” as a read-only Personal Wiki source?`,
      {
        modal: true,
...
async function configurePersonalWikiLibrary(context) {
  ...
  if (choice.action === 'register') return registerExternalStore(context, folder);
  ...
  if (choice.action === 'remove') {
    ...
    const approved = context.extensionMode === vscode.ExtensionMode.Test
      ? true
      : await vscode.window.showWarningMessage(
        `Remove “${selected.label}” from Personal Wiki Library?`,
    ...
    await library.removeStore(context, selected.storeId);
    return true;
  }
  return undefined;
}
...
function registerPersonalWikiLibraryCommand(context) {
  context.subscriptions.push(vscode.commands.registerCommand(COMMAND, () => configurePersonalWikiLibrary(context)));
}
