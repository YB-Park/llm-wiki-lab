...
assert.equal(commands.size, 16, '0.1.9 command surface count changed unexpectedly');

const tools = manifest.contributes.languageModelTools || [];
assert.equal(tools.length, 2, '0.1.9 must expose exactly two bounded Agent Wiki tools');
...
assert.match(searchTool.modelDescription, /read-only/i);
...
assert.match(rememberTool.modelDescription, /cannot perform correction, change, dispute, supersession, deletion/i);
...
assert(agentTools.includes("['discover', query, '--top-k-per-topic', '3', '--json']"), 'read tool must use deterministic current-view discovery');
assert(agentTools.includes("authority=read_only"), 'read tool result must state read-only authority');
assert(agentTools.includes("canonical_mutation=none"), 'read tool result must state no canonical mutation');
assert(agentTools.includes("['ingest', target.filePath, '--topic', topic.id]"), 'remember tool may perform only explicit source ingest');
assert(agentTools.includes('confirmationMessages'), 'source admission must have explicit tool confirmation context');
assert(agentTools.includes('model_calls=0'), 'slice-1 remember tool must state zero model calls');
assert(agentTools.includes('human_authorship_persisted=no'), 'source admission must not become human-authorship persistence');
assert(agentTools.includes('canonical_semantic_mutation=none'), 'source admission must not silently perform epistemic mutation');
assert(agentTools.includes('derived_agent_wiki_maintenance=not_run_in_slice_1'), 'slice boundary must remain explicit');
assert(!agentTools.includes("'--allow-model-call'"), 'Agent Wiki slice-1 tools must never authorize a model call');
assert(!agentTools.includes("'source', 'correct'"), 'remember tool must not expose correction');
assert(!agentTools.includes("'source', 'change'"), 'remember tool must not expose change');
assert(!agentTools.includes("'source', 'dispute'"), 'remember tool must not expose dispute');
assert(!agentTools.includes("'source', 'supersede'"), 'remember tool must not expose supersession');
assert(!agentTools.includes('persist_inferred_human_commitment'), 'slice-1 tools must not manufacture Human Knowledge authorship');
...
console.log('VS-CODE-DOGFOOD-STATIC PASS version=0.1.9 commands=16 agentTools=2 ambientRead=bounded-current explicitRemember=ingest-only toolModelCalls=0 humanAuthorshipProtected=yes semanticMutationProtected=yes python39Compat=required knowledgeNote=human-owned-no-auto-mutation citationBoundary=core-bundled doctorModelCalls=0 gitSafety=read-only bundledCore=generated compiledProvider=not-implemented');
