...
def _require_initialized(root: Path) -> None:
    if not (
        ...
        raise RuntimeError("replica_not_initialized")


def _require_snapshot(root: Path, expected: str) -> None:
    if not SNAPSHOT_ID_RE.fullmatch(expected):
        raise RuntimeError("replica_snapshot_identity_changed")
    if snapshot_manifest(root)["snapshot_id"] != expected:
        raise RuntimeError("replica_snapshot_identity_changed")
...
def _read_agent_state_readonly(root: Path) -> dict:
    ...
    return _validate(json.loads(state_path.read_text(encoding="utf-8")))
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="llm-wiki-replica-read")
...
def main(argv: list[str] | None = None) -> int:
    ...
    expected = args.expected_snapshot_id
    try:
        ...
        raise SystemExit("REPLICA-READ-STOP replica_read_failed") from None
